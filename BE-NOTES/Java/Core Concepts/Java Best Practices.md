...
|---|---|---|---|
| Stringhe confrontate con `==` | `if (a == b)` non funziona anche se le stringhe sono identiche | `==` confronta riferimenti, non contenuto | Usa `a.equals(b)` o `Objects.equals(a, b)` |
| Modifica collezione durante iterazione for-each | `ConcurrentModificationException` a runtime | Il for-each usa un iterator interno; `list.remove()` lo invalida | Usa `iterator.remove()`, `removeIf()`, o colleziona in una lista separata |
| Risorse non chiuse (file, connessioni) | Resource leak, file bloccati, timeout connessioni | `FileInputStream` non chiuso in caso di eccezione | Usa try-with-resources: `try (var f = new FileInputStream(...))` |
| Shadowing di variabile | Il codice usa la variabile sbagliata senza warning | Parametro o variabile locale ha lo stesso nome di un campo | Rinominare o usare `this.field` esplicitamente |
| ArrayList.subList() restituisce una *view*, non una copia | `list.subList(0, 2).clear()` modifica la lista originale | `subList()` è una view — operazioni su di essa si riflettono sulla sorgente | Usare `new ArrayList<>(list.subList(...))` per una copia indipendente |
| `float` / `double` per calcoli monetari | Arrotondamenti imprecisi (es. `0.1 + 0.2 != 0.3`) | Floating point binario non può rappresentare esattamente decimali come 0.1 | Usa `BigDecimal` per valute |
| Array covariante | `String[]` è sottotipo di `Object[]`, ma a runtime si rompe | `Object[] o = new String[1]; o[0] = 42;` → `ArrayStoreException` | Preferire `List<T>` generica (invariante) |
| `switch` senza `break` (pre-Java 14) | Esecuzione di più case consecutivi (fall-through) | Ogni case "cade" nel successivo se manca `break` | Java 14+ usa `switch` expression con freccia `->` |
| Metodo `finalize()` per cleanup | Comportamento imprevedibile, performance degradata | `finalize()` è deprecato da Java 9, esecuzione non deterministica | Usa `Cleaner`, `AutoCloseable`, o try-with-resources |
| `@Override` mancante | Firma di metodo non corrisponde a quella del genitore, ma compila | Intendi fare override ma hai sbagliato nome o parametri | Aggiungi sempre `@Override` su ogni metodo che intendi ridefinire |

---

## Riferimenti

- [[BE-NOTES/Java/Core Concepts/Variabili e Tipi di Dati]]
- [[BE-NOTES/Java/Core Concepts/Generics]]
